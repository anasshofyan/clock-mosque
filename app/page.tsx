...
export default function Home() {
  ...
  >([]);
  const [mosqueStaff, setMosqueStaff] = useState<{
    muazzin: { name: string; location: string };
    imam: { name: string; location: string };
  } | null>(null);
  ...
  useEffect(() => {
    const loadMosqueData = async () => {
      try {
        ...
        const prayerService = PrayerTimeService.getInstance();
        const cityData = await prayerService.getCityById(
          configData.mosque.cityCode
        );

        // Update city info untuk header
        setCityInfo({
        ...
        // Load prayer schedules (muadzin/imam)
        if (configData.mosque.prayerSchedules) {
          console.log('Loading prayer schedules:', configData.mosque.prayerSchedules);
          setPrayerSchedules(configData.mosque.prayerSchedules);
        } else {
          console.log('No prayerSchedules found in config');
        }

        // Load mosque staff dari localStorage
        const staffData = storage.getItem('mosque_staff');
        if (staffData) {
          try {
            const parsedStaff = JSON.parse(staffData);
            console.log('Loading mosque staff:', parsedStaff);
            setMosqueStaff(parsedStaff);
          } catch (e) {
            console.error('Error parsing mosque_staff:', e);
          }
        }
    ...
    // Polling untuk mengecek update setiap 10 detik
    const pollInterval = setInterval(async () => {
      try {
        const config = await getConfigService();
        // Force refresh dari API (bypass localStorage cache)
        const freshConfig = await config.refreshConfigFromAPI();

        console.log("Polling: Checking for updates...");

        // Update state langsung dengan data fresh
        const prayerService = PrayerTimeService.getInstance();
        const cityData = await prayerService.getCityById(
          freshConfig.mosque.cityCode
        );

        // Update city info untuk header
        setCityInfo({
          id: freshConfig.mosque.cityCode,
          lokasi: cityData.lokasi
        });

        setMosqueInfo({
          name: freshConfig.mosque.name,
          location: cityData.lokasi,
          cityCode: freshConfig.mosque.cityCode,
          about: freshConfig.mosque.about,
          liveStream: freshConfig.mosque.liveStream || {
            url: "",
            title: "Masjidil Haram",
            description: "Live from Makkah",
            autoplay: true,
        ...
        // Update prayer schedules
        if (freshConfig.mosque.prayerSchedules) {
          console.log('Polling: Updating prayer schedules:', freshConfig.mosque.prayerSchedules);
          setPrayerSchedules(freshConfig.mosque.prayerSchedules);
        }
        ...
        // Update kajian/announcements langsung (force refresh sudah dilakukan di atas)
        await loadAnnouncements();

        console.log("Config updated and state refreshed!");
      } catch (error) {
        console.error("Error polling config updates:", error);
      }
    }, 10000); // Cek setiap 10 detik

    // Listen untuk storage event dari tab lain (cross-tab sync)
    const handleStorageChange = async (e: StorageEvent) => {
      if (
        ...
        // Reload langsung dari localStorage yang baru saja berubah
        try {
          const config = await getConfigService();
          const configData = await config.getConfig();
          ...
          await loadAnnouncements();
          console.log("State updated from storage change!");
        } catch (error) {
  ...
  useEffect(() => {
    ...
    const calculateOptimalInterval = () => {
      ...
      let nextPrayerTime = prayerTimesInMinutes.find(
      ...
      const minutesUntilNextPrayer =
        nextPrayerTime.minutes - currentTimeMinutes;
    ...
    const checkPrayerTime = () => {
      ...
      prayerTimes.forEach((prayer) => {
        ...
        const isWithinWindow =
          Math.abs(currentTimeMinutes - prayerTimeMinutes) <= 1;
        ...
        if ((isWithinWindow || isExactMatch) && !wasRecentlyTriggered) {
          ...
          setTimeout(() => {
            setRecentlyTriggered((prev: string[]) =>
              prev.filter((name) => name !== prayer.name)
            );
          }, 5 * 60 * 1000);
        ...
        <div className="container h-full mx-auto px-4 flex items-center justify-between relative">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg md:text-xl lg:text-2xl font-medium text-[#E6D5C9]">
                {formatDate()}
              </h2>
            </div>
            <h2 className="text-sm md:text-base lg:text-lg text-[#E6D5C9]/70 ml-3">
              {cityInfo.lokasi}
            </h2>
          </div>
          ...
          <div className="absolute left-1/2 -translate-x-1/2 text-center top-1/2 -translate-y-1/2">
            {mosqueInfo.name && (
              <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-[#E6D5C9]">
                {mosqueInfo.name}
              </h1>
            )}
          </div>
            ...
            <div className="space-y-3 flex flex-col h-full">
              <div className="bg-[#1F2A24]/80 backdrop-blur-sm rounded-lg p-3 border border-[#E6D5C9]/10">
                ...
                <div className="bg-[#2D3B35]/80 rounded-lg p-3 border border-[#E6D5C9]/10">
                  <p className="text-lg md:text-xl lg:text-2xl font-semibold text-[#E6D5C9]">
                    {mosqueStaff?.muazzin?.name || prayerSchedules.find(s => s.muadzin)?.muadzin || 'Belum diatur'}
                  </p>
                  <p className="text-xs md:text-sm text-[#E6D5C9]/70">
                    {mosqueStaff?.muazzin?.location || mosqueInfo.location || 'Lokasi'}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-[#1F2A24]/80 backdrop-blur-sm rounded-lg p-3 border border-[#E6D5C9]/10">
                ...
                <div className="bg-[#2D3B35]/80 rounded-lg p-3 border border-[#E6D5C9]/10">
                  <p className="text-lg md:text-xl lg:text-2xl font-semibold text-[#E6D5C9]">
                    {mosqueStaff?.imam?.name || prayerSchedules.find(s => s.imam)?.imam || 'Belum diatur'}
                  </p>
                  <p className="text-xs md:text-sm text-[#E6D5C9]/70">
                    {mosqueStaff?.imam?.location || mosqueInfo.location || 'Lokasi'}
                  </p>
                </div>
              </div>
                      ...
                      <div
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-xs text-[#E6D5C9]/70">
                              {kajian.ustadz}
                            </span>
                          </div>
              ...
              {mosqueInfo?.liveStream?.url ? (
                <iframe
                  src={`${mosqueInfo.liveStream.url}${
                    ...
                    mosqueInfo.liveStream.autoplay ? "1" : "0"
                  }&mute=${mosqueInfo.liveStream.muted ? "1" : "0"}`}
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                  allowFullScreen
                  className="w-full h-full object-cover min-h-0 flex-1"></iframe>
              ) : (
                <Image
                  src="/makkah.jpg"
                  alt="Makkah Live"
      ...
      <footer className="h-24 flex-shrink-0 container mx-auto px-4 py-2">
        {isLoading ? (
          <div className="h-full flex items-center justify-center">
            <p className="text-[#E6D5C9]/70 animate-pulse">
          ...
          <div className="grid grid-cols-6 gap-2 h-full">
            {prayerTimes.map((prayer) => (
              <div
                ...
                <p
                  className={`text-lg md:text-xl lg:text-2xl font-mono font-bold ${
                    prayer.name === nextPrayer.name
                      ? "text-[#E6D5C9]"
                      : "text-[#E6D5C9]/90"
                  }`}>
